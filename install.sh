...
if [[ -z "${HOMEBREW_ON_LINUX-}" ]]; then
  ...
  if [[ "$UNAME_MACHINE" == "arm64" ]]; then
    ...
    # On Intel macOS, this script installs to /usr/local/homebrew only
...
REQUIRED_RUBY_VERSION=2.6  # https://github.cmom/Homebrew/brew/pull/6556
...
if should_install_command_line_tools; then
  ...
  ohai "If Xcode is already installed, you don’t need to install Xcode Command Line Tools."
fi
...
# Fallback to original 'xcode-select' method
if should_install_command_line_tools; then
